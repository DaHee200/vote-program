...
interface VoteResultRepository : JpaRepository<VoteResult, Long> {
    ...
    @Modifying
    @Query("UPDATE VoteResult v SET v.agreeCount = v.agreeCount + 1, v.totalCount = v.totalCount + 1 WHERE v.post.id = :postId")
    ...
    @Modifying
    @Query("UPDATE VoteResult v SET v.disagreeCount = v.disagreeCount + 1, v.totalCount = v.totalCount + 1 WHERE v.post.id = :postId")
